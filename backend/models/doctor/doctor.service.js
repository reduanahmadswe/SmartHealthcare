...
const doctorService = {
     ...
     */
    getDoctorsPendingVerification: async ({
        page = 1,
        limit = 10
    }) => {
    ...
    getDoctorAppointments: async (doctorId, currentUser, filters) => {
        ...
        if (currentUser._id.toString() !== doctorId && currentUser.role !== 'admin') {
    ...
    getAllVerifiedDoctors: async () => {
        ...
        return doctors.map(doc => ({
            ...
            fee: (doc.doctorInfo && doc.doctorInfo.consultationFee) ? doc.doctorInfo.consultationFee : 0,
            specialization: (doc.doctorInfo && doc.doctorInfo.specialization) ? doc.doctorInfo.specialization : [],
        }));
    },
