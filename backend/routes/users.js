...
const { authenticateToken, requirePatient, requireOwnership } = require('../middleware/auth');
...
// @access  Private
router.post('/profile-picture', asyncHandler(async (req, res) => {
  if (!req.file) {
    return res.status(400).json({
      success: false,
...
router.get('/health-data', requirePatient, asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id);
...
// @access  Private (Patient only)
router.put('/health-data', requirePatient, [
  ...
  const allowedFields = [
    ...
    'chronicDiseases', 'currentMedications', 'familyHistory'
  ];
