...
const medicalRecordService = {
  ...
  uploadRecord: async (recordData, file, currentUser) => {
    ...
    // Add history entry
    medicalRecord.addToHistory(
      "created",
      currentUser._id,
      `Record of type ${recordType} created.`
    );
  ...
  getRecordById: async (recordId, currentUser) => {
    ...
    const isOwner =
      medicalRecord.patient._id.toString() === currentUser._id.toString();
    const isUploader =
      medicalRecord.uploadedBy._id.toString() === currentUser._id.toString();
    const isShared = medicalRecord.sharedWith.some(
      (share) =>
        share.doctor &&
        share.doctor._id.toString() === currentUser._id.toString()
    );
    const isDoctorConsulting =
      medicalRecord.doctor &&
      medicalRecord.doctor._id.toString() === currentUser._id.toString(); // If they are the consulting doctor
    ...
    medicalRecord.addToHistory(
      "viewed",
      currentUser._id,
      `Record viewed by ${currentUser.role}`
    );
  ...
  updateRecord: async (recordId, updateData, currentUser) => {
    ...
    const isOwner =
      medicalRecord.patient.toString() === currentUser._id.toString();
    const isUploader =
      medicalRecord.uploadedBy.toString() === currentUser._id.toString();
    const isAdmin = currentUser.role === "admin";

    // Doctors might also have 'edit' permission via sharedWith
    const sharedAccess = medicalRecord.sharedWith.find(
      (share) =>
        share.doctor &&
        share.doctor.toString() === currentUser._id.toString() &&
    ...
    await medicalRecord.addToHistory(
      "updated",
      currentUser._id,
      `Record updated by ${currentUser.role}`
    );
  ...
  deleteRecord: async (recordId, currentUser) => {
    ...
    await medicalRecord.addToHistory(
      "deleted",
      currentUser._id,
      `Record marked as deleted by ${currentUser.role}`
    );
    await medicalRecord.save();
  ...
  shareRecordWithDoctor: async (
    ...
    await medicalRecord.addToHistory(
      "shared",
      currentUser._id,
      `Record shared with doctor ${doctor.firstName} (ID: ${doctorId})`
    );

    // Find the newly added/updated share entry to return
    const updatedShareInfo = medicalRecord.sharedWith.find(
      (share) => share.doctor.toString() === doctorId.toString()
    );
  ...
  unshareRecordWithDoctor: async (medicalRecordId, doctorId, currentUser) => {
    ...
    await medicalRecord.addToHistory(
      "unshared",
      currentUser._id,
      `Access revoked for doctor (ID: ${doctorId})`
    );
  },
  ...
  downloadRecord: async (recordId, currentUser) => {
    ...
    const isDoctorConsulting =
      medicalRecord.doctor &&
      medicalRecord.doctor._id.toString() === currentUser._id.toString();
    const isAdmin = currentUser.role === "admin";
    ...
    if (
      ...
      throw new Error(
        "Access denied: You do not have permission to download this record."
      );
    }
    ...
    medicalRecord.addToHistory(
      "downloaded",
      currentUser._id,
      `Record downloaded by ${currentUser.role}`
    );
    // No await here as it's a non-critical background update
  ...
  getPatientRecordsByDoctor: async (patientId, filters, currentUser) => {
    if (currentUser.role !== "doctor" && currentUser.role !== "admin") {
      throw new Error(
        "Access denied: Only doctors and admins can view specific patient records."
      );
    }
