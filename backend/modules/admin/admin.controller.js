...
class AdminController {
  ...
  static async getPendingDoctors(req, res, next) {
    try {
      try {
        ...
        res
          .status(500)
          .json({ success: false, message: "Error fetching doctors" });
      }
    } catch (error) {
      next(error);
    }
  ...
  static async approveDoctor(req, res, next) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Error approving doctor" });
    }
  }
  ...
  static async updateUserVerificationStatus(req, res, next) {
    ...
    try {
      const user = await AdminService.updateUserVerificationStatus(
        userId,
        status,
        reason,
        adminId
  ...
  static async getDetailedAnalytics(req, res, next) {
    ...
    try {
      const analyticsData = await AdminService.getDetailedAnalytics(
        start,
        end,
        groupBy
      );
  ...
  static async getUnverifiedDoctors(req, res, next) {
    ...
    try {
      const { doctors, pagination } = await AdminService.getUnverifiedDoctors({
        page,
        limit,
      });
      res.json({
  ...
  static async verifyDoctor(req, res, next) {
    ...
    try {
      ...
      if (
        ...
        error.message.includes("not a doctor") ||
  ...
  static async rejectDoctorVerification(req, res, next) {
    ...
    try {
      const doctor = await AdminService.rejectDoctorVerification(
        doctorId,
        rejectionReason
      );
      res.json({
  ...
  static async getSystemLogs(req, res, next) {
    ...
    try {
      const { logs, pagination } = await AdminService.getSystemLogs({
        level,
        startDate,
        endDate,
        page,
