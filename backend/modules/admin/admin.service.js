...
class AdminService {
    ...
    const revenueByDoctor = await Appointment.aggregate([
      ...
      },
      {
        $group: {
          _id: "$doctor._id",
          doctorName: {
            $first: { $concat: ["$doctor.firstName", " ", "$doctor.lastName"] },
          },
  ...
  static async verifyDoctor(doctorId, verificationNotes, adminId) {
    ...
    await sendEmail({
      ...
      template: "doctorVerification", // 👈 use doctorVerification.hbs
      context: {
        name: `${doctor.firstName} ${doctor.lastName}`,
        verificationNotes:
          verificationNotes || "Your account has been verified successfully.",
      },
    });
