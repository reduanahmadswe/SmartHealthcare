...
appointmentSchema.pre('save', function(next) {
    ...
    // Only apply this validation for new appointments, or if the date field is modified
    if (this.isNew || this.isModified('appointmentDate') || this.isModified('appointmentTime')) {
        ...
        appointmentDateTime.setHours(hours, minutes, 0, 0); // Set time to compare full datetime
...
appointmentSchema.statics.checkAvailability = async function(doctorId, date, time, duration = 30) {
    const conflictingAppointment = await this.findOne({
        doctor: doctorId,
        appointmentDate: date, // Comparing dates directly
        appointmentTime: time, // Comparing times directly
        status: {
            $in: ['pending', 'confirmed', 'in_progress']
        } // Include 'in_progress' to prevent overlapping ongoing appointments
    });
...
appointmentSchema.statics.getStatistics = async function(doctorId, startDate, endDate) {
    const stats = await this.aggregate([{
        $match: {
            doctor: new mongoose.Types.ObjectId(doctorId), // Use new mongoose.Types.ObjectId
            appointmentDate: {
                $gte: startDate,
                $lte: endDate
            }
