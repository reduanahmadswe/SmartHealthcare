...
const { uploadPrescriptionPDF } = require('../../utils/cloudinaryService');
...
class PrescriptionService {
  static async createPrescription(prescriptionData, doctorId) {
    ...
    if (appointment.doctor._id.toString() !== doctorId.toString()) {
      const error = new Error(
        "Access denied. You can only create prescriptions for your appointments."
      );
    ...
    const prescriptionNumber = `RX-${new Date().getFullYear()}-${String(
      prescriptionCount + 1
    ).padStart(5, "0")}`;
  ...
  static async getPrescriptions(user, { status, page, limit }) {
    const query = {};
  ...
  static async updatePrescription(prescriptionId, updateData, doctorId) {
    ...
    if (
      ...
      updateData.patientInstructions ||
      updateData.labTests ||
      updateData.lifestyleRecommendations
    ) {
      const pdfBuffer = await generatePrescriptionPDF(prescription);
      const uploadResult = await uploadPrescriptionPDF(
        pdfBuffer,
      ...
      prescription.pdfUrl = uploadResult.url;
      prescription.pdfGeneratedAt = new Date();
    }
  ...
  static async getPrescriptionsForPatient(
    ...
    if (
      reqUserRole === "doctor" &&
      patientId.toString() !== reqUserId.toString()
    ) {
  ...
  static async getPrescriptionStatistics(doctorId, { startDate, endDate }) {
